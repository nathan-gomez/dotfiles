return {
  ...
  config = function()
      ...
      callback = function()
        if vim.fn.argc() == 0 then
          ...
          if vim.uv.fs_stat(session_file) then
            vim.schedule(function() sessions.read(default_name) end)
          end
        end
      end,
    })
    ...
    local function delete_session()
      ...
      local names = vim.tbl_map(function(s) return s.name end, sessions_tbl)
      vim.ui.select(vim.tbl_values(names), { prompt = "Delete session:" },
        function(choice)
          if choice then
    ...
    vim.keymap.set("n", "<leader>qc", function() sessions.write(default_name) end, { desc = "Sessions: Write local" })
    vim.keymap.set("n", "<leader>ql", function() sessions.select() end, { desc = "Sessions: Select" })
